...
const inputClass = 'h-[3.25rem] rounded-[1.15rem] border border-black/10 bg-white px-4 text-base font-bold text-black outline-none transition placeholder:text-black/35 focus:border-[var(--slot4-accent-fill)] focus:ring-4 focus:ring-[var(--slot4-accent-fill)]/10'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-[1.15rem] bg-[var(--slot4-accent-fill)] px-6 text-sm font-black uppercase text-white transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(139,0,255,0.24)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      <button type="submit" className={buttonClass}>Enter newsroom</button>
    </form>
  )
}
...
export function EditableLocalSignupForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      <input className={inputClass} type="password" placeholder="Password" value={password} onChange={(event) => setPassword(event.target.value)} required />
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
      <button type="submit" className={buttonClass}>Create reader profile</button>
    </form>
  )
}
